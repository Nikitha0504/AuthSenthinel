...
async function seed() {
  try {
    await mongoose.connect(process.env.MONGO_URI || 'mongodb://127.0.0.1:27017/authsentinel');
    ...
    // Note: We bypass pre-save hook for seed by not providing plain text to compare later or we can let model hash it
    defaultUser.passwordHash = 'password123'; 
    await defaultUser.save(); // Model will hash this properly
    ...
    console.log(`[Seed] Injected ${logs.length} logs and ran alerts engine.`);
